...
using SV22T1020214.DataLayers.Interfaces;
using SV22T1020214.Models.Common;
using SV22T1020214.Models.Sales;
using System.Data;

namespace SV22T1020214.DataLayers.SqlServer
{
    /// <summary>
            ...
            using (var connection = GetConnection())
            {
                var sql = @"SELECT COUNT(*) FROM Orders
                            WHERE (@Status = 0 OR Status = @Status)
                                ...
                                AND (@CustomerId IS NOT NULL OR @SearchValue = N'' OR DeliveryProvince LIKE @SearchValue OR DeliveryAddress LIKE @SearchValue);
                            ...
                            WHERE (@Status = 0 OR o.Status = @Status)
                                ...
                                AND (@CustomerId IS NOT NULL OR @SearchValue = N'' OR o.DeliveryProvince LIKE @SearchValue OR o.DeliveryAddress LIKE @SearchValue)
